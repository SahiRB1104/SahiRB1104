...
def generate_info_card_svg(output_path="info-card.svg"):
    lines = [
        ("Host", "Sahil Bhalekar (Software Developer)", "#7ee787"),
        ("Primary Focus", "AI/ML, LLM Integration & Async Systems", "#58a6ff"),
        ("Core Stack", "Python, React.js, FastAPI, Node.js, Express", "#79c0ff"),
        ("AI / ML Pipeline", "HuggingFace Transformers, NLP, Sentiment/Fake News Models", "#d2a8ff"),
        ("Databases", "PostgreSQL, MongoDB, Redis (15-min TTL Caching)", "#ffa657"),
        ("Cloud & DevOps", "Docker, GCP Cloud Run, AWS, Vercel", "#58a6ff"),
        ("DSA & Problem Solving", "Java (Active DSA Practice & Algorithms)", "#7ee787"),
        ("Certifications", "OCI 2025 AI Foundation (Oracle), AWS Cloud Practitioner", "#f0883e"),
        ("Featured Work", "NewsAura (AI News Platform), Atomic Audit Log Engine", "#79c0ff"),
    ...
    padding_y = 50
    card_width = 540
    card_height = padding_y + len(lines) * line_height + 25
    ...
    svg.append('''
        ...
        .label { font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 11.5px; fill: #8b949e; font-weight: 600; }
        .val { font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 11.5px; font-weight: 500; }
        .prompt-symbol { font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 12px; fill: #7ee787; font-weight: 700; }
    ...
    for idx, (label, val, val_color) in enumerate(lines):
        ...
        svg.append(f'    <text class="label" x="16" y="{y_pos}">{label}:</text>')
        # Calculate label offset
        label_width = len(label) * 7.5 + 24
        svg.append(f'    <text class="val" x="{label_width}" y="{y_pos}" fill="{val_color}">{val}</text>')
        svg.append('  </g>')
    svg.append('</g>')
    
    # Color palette bar at bottom (neofetch style)
